...
export default function MuroAficion({
  initialMessages,
  currentUserEmail,
}: {
  initialMessages: FanMessage[];
  ...
  return (
    <div className="flex flex-col h-[500px] bg-white dark:bg-slate-900/50 border border-primary/10 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden transition-colors duration-300 backdrop-blur-sm relative">
      ...
      <div className="bg-primary px-4 py-3 border-b border-primary/20 flex items-center gap-2">
        ...
        <h3 className="font-lexend font-bold text-on-primary text-sm uppercase tracking-wide">
          Sala de Debate Live
        </h3>
      ...
      >
        {messages.length === 0 ? (
          <p className="text-center text-slate-500 dark:text-slate-400 text-sm mt-10 font-body-md">
            Sé el primero en escribir en el muro...
          </p>
        ) : (
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                ...
                <div
                  className={`px-4 py-2 rounded-2xl max-w-[85%] font-body-md shadow-sm ${
                    isMe
