                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-3 w-48 bg-surface-container-lowest dark:bg-slate-900 border border-outline-variant/20 dark:border-slate-800 rounded-lg shadow-lg overflow-hidden flex flex-col z-50">
                      <div className="px-4 py-3 border-b border-outline-variant/20 dark:border-slate-800">
                        <p className="text-[10px] uppercase font-bold text-outline dark:text-slate-500 tracking-wider">
                          Conectado como
                        </p>
                        <p
                          className="text-xs font-medium text-on-surface dark:text-slate-300 truncate"
                          title={user.email}
                        >
                          {user.email}
                        </p>
        ...
        <div className="fixed inset-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg pt-24 px-6 flex flex-col md:hidden animate-in fade-in duration-200">
          <nav className="flex flex-col gap-2">
            <Link
              ...
              className="w-full text-center py-4 font-label-md text-lg text-slate-600 dark:text-slate-300 min-h-[44px] border-b border-outline-variant/20 hover:bg-surface-container-low transition-colors"
            >
              Partidos
            </Link>
            <Link
              href="#"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full text-center py-4 font-label-md text-lg text-blue-800 dark:text-blue-400 font-bold bg-blue-50 dark:bg-slate-800 rounded min-h-[44px] hover:bg-blue-100 dark:hover:bg-slate-700 transition-colors"
            >
              Noticias
            </Link>
            <Link
            ...
            <Link
              href="/comunidad"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full text-center py-4 font-label-md text-lg text-slate-600 dark:text-slate-300 min-h-[44px] hover:bg-surface-container-low transition-colors"
            >
              Comunidad
            </Link>
